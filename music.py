...
class pianoTrans():
    ...
    def write_xls(self, save_path, file_name, results):
        ...
        :return:
        '''
        index = len(results)  # 获取需要写入数据的行数
        workbook = xlrd.open_workbook(os.path.join(save_path, file_name))  # 打开工作簿
        ...
        for i in range(0, index):
            for j in range(0, len(results[i])):
                new_worksheet.write(i + rows_old, j, results[i][j])  # 追加写入数据，注意是从i+rows_old行开始写入
        new_workbook.save(os.path.join(save_path, file_name))  # 保存工作簿
